...
export default class Chat {
       constructor(element, socket, receive) {
              ...
              this.activeUserName =
                     document.querySelector("#activeUserName").textContent;
              this.index = 10;
       }
       ...
       async chatHeader() {
              let payload = {
                     sender: document.getElementById("activeUserName")
                            .textContent,
              ...
              let dataMessage = JSON.parse(
                     await requestChat(this.socket, payload)
              );
              this.datamessage = {
              ...
              this.socket.onmessage = async (event) => {
                     ...
                     let username = this.receive.substring(9);
                     if (value.type !== "typing") {
                            this.updatedChatBox(value);
                     } else if (
                            value.type == "typing" &&
                            value.receiver == this.activeUserName
                     ) {
       ...
       async updatedChat(receiver) {
              ...
              this.updateSeenMessage(receivedMessages);
       }
       ...
       async printChat(receiver) {
              ...
              this.withReceiver.slice(-this.index).forEach((message) => {
                     ...
                     if (
                            ...
                            messageBox.style.justifyItems = "end";
                            messageInfo.classList.add("mInfo");
                            messageInfo.style.float = "right";
                            messageInfo.innerHTML = `<b>Me:</b> ${message.time}`;
                     } else if (
                            message.receiver == this.activeUserName &&
                            message.sender == receiver
                     ) {
                            messageBox.classList.add("messageBox");
                            messageBox.style.justifyItems = "start";
                            messageInfo.classList.add("mInfo");
                            messageInfo.style.float = "left";
                            messageInfo.innerHTML = `<b>${message.sender}:</b> ${message.time}`;
                     }
       ...
       async updatedChatBox(message) {
              if (
                     ...
                     if (message != undefined) {
                            ...
                            if (
                                   this.message.sender == this.activeUserName ||
                                   this.message.receiver == this.activeUserName
                            ) {
                                   await this.updatedChat(
                                          document.getElementById(
                                                 "receiverName"
                                          ).textContent
                                   );
                                   const chatContainer = await this.printChat(
                                          document.getElementById(
                                                 "receiverName"
                                          ).textContent
                                   );
       ...
       async addOldMessage() {
              ...
              parent.innerHTML = await this.printChat(
