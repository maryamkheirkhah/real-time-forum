...
/* func CheckCookieExpiration(sessionId string, cookie *http.Cookie, sessions *LoggedInSessions, maxIdleTime time.Duration) {
	// Wait for the cookie to expire
	time.Sleep(cookie.Expires.Sub(time.Now()))

	// Check if the session is still active
	_, ok := sessions.Data[sessionId]
	if !ok {
		return
	}

	// Clean up the session if it has been idle for too long
	sessions.CleanUpInactiveSessions(maxIdleTime)
}
func CheckAllCookieExpiration(r *http.Request, sessions *LoggedInSessions, maxIdleTime time.Duration) {
	for sessionId, _ := range sessions.Data {
		cookie, err := r.Cookie("session")
		if err != nil {
			continue
		}
		if cookie.Value == sessionId {
			go CheckCookieExpiration(sessionId, cookie, sessions, maxIdleTime)
		}
	}
}
*/
